...
def _thick_bottom(color=NAVY) -> Border:
    side = Side(style="medium", color=color)
    return Border(bottom=side)

def _cell(ws, row, col, value, bold=False, size=11, color=NAVY,
...
def _merge_title(ws, row, start_col, end_col, value, bg=NAVY, fg=WHITE,
...
def _section_header(ws, row, start_col, end_col, value, bg=NAVY, fg=WHITE,
                    size=10):
    ws.merge_cells(
        start_row=row, start_column=start_col,
        end_row=row,   end_column=end_col
...
def _build_roi_sheet(ws, business_name: str, category: str, city: str,
    ...
    # ── Assumptions block ─────────────────────────────────────────────────────
    _section_header(ws, 4, 1, 4, "  📊  KEY ASSUMPTIONS  (blue = change these)", BLUE, WHITE, 9)
    ...
    for i, (label, _, default, fmt) in enumerate(assumptions):
        row = 5 + i
        _cell(ws, row, 1, label, size=9, color=GRAY_700)
        c = ws.cell(row=row, column=2, value=default)
        c.font         = _font(True, 10, BLUE)
        c.alignment    = _align("center", "center")
        c.number_format = fmt
    ...
    # ── Projected results ─────────────────────────────────────────────────────
    _section_header(ws, 12, 1, 4, "  📈  PROJECTED RESULTS  (auto-calculated)", GREEN, WHITE, 9)
    ...
    for i, (label, formula, fmt) in enumerate(results):
        ...
        c.font          = _font(True, 10, NAVY)
        c.alignment     = _align("center", "center")
        c.number_format = fmt
        c.fill          = _fill(GRAY_100)
    ...
    ws.cell(row=22, column=1, value="Month").font = _font(True, 9, GRAY_700)
    ws.cell(row=22, column=2, value="Cumulative Revenue Uplift (₹)").font = _font(True, 9, GRAY_700)
    ws.cell(row=22, column=3, value="Investment").font = _font(True, 9, GRAY_700)
    ...
    chart.series[1].graphicalProperties.solidFill = AMBER
...
def _build_competitor_sheet(ws, business_name: str, category: str):
    ...
    _merge_title(ws, 1, 1, 7, f"Competitor Digital Snapshot  —  {category}", NAVY, WHITE, 14)
    ...
    score_fills = {"High": GREEN, "Medium": AMBER, "Low": "EF4444"}
    for ri, row_data in enumerate(rows):
        row_num   = 5 + ri
        row_fill  = LIGHT_BLUE if ri == 0 else (GRAY_100 if ri % 2 == 0 else WHITE)
        bold_row  = ri == 0
        for ci, val in enumerate(row_data):
            c = ws.cell(row=row_num, column=ci + 1, value=val)
            c.font      = _font(bold_row, 9, NAVY)
    ...
    note.font      = _font(False, 8, GRAY_700, italic=True)
...
def _build_roadmap_sheet(ws, benefits: List[str]):
    ...
    _merge_title(ws, 1, 1, 6, "Project Roadmap & Deliverables", NAVY, WHITE, 14)
    ...
    for phase in phases:
        ...
        for di, deliverable in enumerate(phase["deliverables"]):
            show_phase = di == 0
            r_fill     = GRAY_100 if di % 2 == 0 else WHITE

            # Phase name cell (only first row of each phase)
            c = ws.cell(row=row, column=1,
                        value=phase["phase"] if show_phase else "")
            c.font      = _font(True, 9, WHITE if show_phase else NAVY)
            ...
            c2.font      = _font(False, 9, GRAY_700)
            ...
            c3.font      = _font(False, 9, GRAY_700)
    ...
    row += 1
    _section_header(ws, row, 1, 6, "  ✅  KEY OUTCOMES THIS PROJECT DELIVERS", GREEN, WHITE, 9)
    row += 1
